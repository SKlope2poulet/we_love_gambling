    ...
    def _au_clic_ROULETTE(self, event):
        ...
        if delete_mode:
            ...
            if key is not None:
                ...
                if removed:
                    self.rafraichir_jetons_ROULETTE(self.app.game.bets)
                    self.app.maj_gains_potentiels_ROULETTE()
...
class RouletteApp(tk.Tk):
    def __init__(self):
        ...
        self.potential_var = tk.StringVar(value="—")  # texte dynamique des gains potentiels
    ...
    def construire_barre_haut_ROULETTE(self):
        ...
        # ---- Ligne des gains potentiels ----
        ttk.Label(top, text="Gains potentiels :").grid(row=1, column=0, sticky="w", pady=(8,0))
        self.potential_lbl = ttk.Label(top, textvariable=self.potential_var, justify="left", wraplength=900)
        self.potential_lbl.grid(row=1, column=1, columnspan=7, sticky="w", pady=(8,0))
    ...
    # ---- Aide gains potentiels ----

    def _libelle_zone_ROULETTE(self, typ, sel):
        if typ == "number":
            return f"Plein {sel}"
        if typ == "corner":
            return "Carré " + "-".join(str(x) for x in sel)
        if typ == "dozen":
            return {1: "1re 12", 2: "2e 12", 3: "3e 12"}.get(sel, "Douzaine")
        if typ == "column":
            return f"Colonne {sel}"
        if typ == "red":
            return "Rouge"
        if typ == "black":
            return "Noir"
    ...
    def _multiplicateur_net_ROULETTE(self, typ):
        # multipliers de profit net (pas retour total)
        return {
            "number": 35,
    ...
    def maj_gains_potentiels_ROULETTE(self):
        """Met à jour le texte dynamique des gains potentiels (par mise + meilleur/pire cas)."""
        if not self.game.bets:
            self.potential_var.set("—")
            return

        # Agrège par (type, selection)
        agg = {}
        ...
        # Par mise (profit net si gagnante)
        parts = []
        total_stake = 0.0
        for (typ, sel), amt in agg.items():
            total_stake += amt
            mult = self._multiplicateur_net_ROULETTE(typ)
            net = mult * amt
            parts.append(f"{self._libelle_zone_ROULETTE(typ, sel)}: +{net:.2f}€")

        # Meilleur / pire cas (net total) en bruteforce sur 0..36
        best_net, best_n = None, None
        worst_net, worst_n = None, None
        r = self.game.roulette
        for n in range(37):
            color = self._couleur_pour_numero_ROULETTE(n)
            net = 0.0
            for b in self.game.bets:
                net += r.gain_pour_mise_ROULETTE(b, n, color)
            if best_net is None or net > best_net:
                best_net, best_n = net, n
            if worst_net is None or net < worst_net:
                worst_net, worst_n = net, n
